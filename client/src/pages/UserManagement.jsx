...
const UserManagement = () => {
    ...
    const [showUnitSelector, setShowUnitSelector] = useState(false); // For Invite Modal
    const [showEditUnitSelector, setShowEditUnitSelector] = useState(false); // For Edit Modal
    ...
    const fetchBlocks = async () => {
        try {
            const blocksRes = await fetch(`${API_URL}/api/properties/blocks`);
            if (blocksRes.ok) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Fetch users
            const usersRes = await fetch(`${API_URL}/api/users?${params.toString()}`);
            ...
            // Fetch blocks independently (or concurrently via separate call if needed, but here sequentially/parallel is fine)
            // Note: We call fetchBlocks() here or in parallel.
            await fetchBlocks();
