        ...
        <div className="glass-card p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 h-full">
            ...
            <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 text-sm text-gray-600 dark:text-neutral-400 items-center">
                    {/* Display all roles */}
                    {activeCommunity?.roles && activeCommunity.roles.length > 0 ? (
                        activeCommunity.roles.map((roleObj, idx) => (
                            <span key={idx} className="px-2 py-0.5 rounded-md bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 text-xs font-bold uppercase tracking-wider">
                                {t(`roles.${roleObj.name}`, roleObj.name)}
                            </span>
                        ))
                    ) : (
                    ...
                    {feeStatus !== 'loading' ? (
                        <Link to="/app/maintenance" className="flex items-center gap-3 px-4 py-3 rounded-xl border border-white/20 dark:border-white/10 backdrop-blur-md transition-all shadow-md hover:shadow-lg bg-white/40 dark:bg-white/5">
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className={`text-xs font-bold uppercase tracking-wider mb-0.5 ${feeStatus === 'paid' ? 'text-green-700 dark:text-green-400' : 'text-orange-700 dark:text-orange-400'
                                    }`}>
